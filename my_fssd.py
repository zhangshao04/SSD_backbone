...
class MYFSSD(nn.Module):
    def __init__(self, cfg):
        ...
        self.conv10_2_extra = nn.ConvTranspose2d(256,1024,kernel_size=3,stride=19,padding=2,output_padding=1)
        self.l2_norm_256 = L2Norm(256, scale=20)
        self.l2_norm_1024 = L2Norm(1024,scale=20)
        self.conv_extra = nn.Conv2d(256,256,kernel_size=3,stride=2,padding=1)
    ...
    def forward(self, x):
        ...
        conv7 = self.l2_norm_256(conv7)
        conv7 = F.relu(conv7,inplace=True)
        fused_feature = torch.cat((s,c,conv7),1)
        ...
        for k, v in enumerate(self.extras):
            ...
            if k == 5:
                conv_10_2 = self.conv10_2_extra(x)
                conv_10_2 = self.l2_norm_1024(conv_10_2)
                conv_10_2 = F.relu(conv_10_2,inplace=True)
                con4_3_new = fused_feature - conv_10_2
                features[0] = con4_3_new
            if k % 2 == 1:
                features.append(x)

        return tuple(features)
        
@registry.BACKBONES.register('MYFSSD')
